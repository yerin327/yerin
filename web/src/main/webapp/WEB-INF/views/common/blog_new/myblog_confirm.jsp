	...
	$( "#dialog-confirm" ).dialog({
	    ...
	    buttons: {
	      "Yes": function() {
	    	  $( this ).dialog( "close" );
	    	  var frm = document.f1;
	    	  frm.action = "<%=request.getContextPath()%>/common/blog_new/myblog_proc.do";
	    	  frm.submit();
	      },
	      "No": function() {
	          $( this ).dialog( "close" );
...
function doBack() {
	var frm = document.f1;
	frm.action = "<%=request.getContextPath()%>/common/blog_new/myblog_enter.do";
	frm.submit();
}
</script>
</head>
		...
		<div class="contents_right blog_confirm">
			...
			<div class="myblog_wrap">
				<div class="listBlogBlock">
					...
					<p class="confirm_article"><c:out value='${contents }'/></p>
				</div>
				<div class="formButtons">
					<ul>
						<li><input type="submit" class="formBt01" value="작성하기" onclick="doSubmit()"></li>
						<li><input type="submit" class="formBt02" value="돌아가기" onclick="doBack()"></li>
					</ul>
				</div>
			</div>
		</div>
