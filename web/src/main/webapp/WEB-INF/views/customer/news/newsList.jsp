...
$(function() {
  $("#add").button({
	  ...
	  frm.action = "/customer/news/newsAddForm.do";
	  frm.submit();
  });
...
function doModify() {
	...
	if (selCount > 1 || selCount == 0) {
		 $( "#dialog-error" ).html("<span class='ui-icon ui-icon-check' style='float:left; margin:0 7px 20px 0;'></span>수정하기 위해서는 하나의 뉴스만 선택해 주세요.");
		 $( "#dialog-error" ).dialog( "open" );
		 return;
	} else {
		var frm = document.newsFrm;
		frm.action = "/customer/news/newsModForm.do";
...
function doRemove() {
	...
	if (selCount == 0) {
		...
		selected_ids = "";
		$( "#dialog-error" ).html("<span class='ui-icon ui-icon-check' style='float:left; margin:0 7px 20px 0;'></span>삭제하려는 뉴스를 선택해 주세요.");
		$( "#dialog-error" ).dialog( "open" );
		return;
	} else {
		...
		console.log(selIds);
		selected_ids = selIds;
		$( "#dialog-confirm" ).html("<span class='ui-icon ui-icon-check' style='float:left; margin:0 7px 20px 0;'></span>선택한 뉴스를 삭제해도 되겠습니까?");
...
<form name="newsFrm" action="" method="post">
	...
	<input type="hidden" name="news_type" value="<%=Constants.NEWS_TYPE_NEWS %>" />
</form>
...
<section class="form_area">
	...
	<div class="tab_area">
    	 <ul>
        	<li><img src="<%=request.getContextPath() %>/images/locale/ja/customer/pages/news/news_on.png" alt="news" width="186" height="50" border="0"></li>
	...
	<div class="form_box">
     	<div id="div_list" style=" width:880px; margin: 0 auto; margin-top:20px; ">
     		<div style="width: 850px; height: 50px; padding-bottom: 10px; text-align: right; ">
	   			<button id="add" style="width:130px; height: 30px; font-size: 10px;">뉴스 추가</button>
	   			<button id="modify" style="width:130px; height: 30px; font-size: 10px;">뉴스 수정</button>
	   			<button id="remove" style="width:130px; height: 30px; font-size: 10px;">뉴스 삭제</button>
   			</div>
		                            ...
		                            <tbody>
		                            	<c:forEach var="news" items="${news_list }" varStatus="status">
			                                ...
			                                <tr class="<c:out value='${rowClass}'/>">
			                                	...
			                                	<c:choose>
				                                	<c:when test="${news.school_id == 'ADMIN' }">
				                                    	<input type="checkbox" name="chkId" value="<c:out value='${news.idx }'/>" disabled="disabled">
				                                    </c:when>
				                                    <c:otherwise>
				                                    	<input type="checkbox" name="chkId" value="<c:out value='${news.idx }'/>">
				                                    </c:otherwise>
			                                    </c:choose>
			                                    </td>
			                                    <td style="width: 50px; vertical-align: baseline;"><c:out value='${news.no}'/></td>
			                                    <td style="width: 480px; ">
			                                    	<a href="javascript:doView(<c:out value='${news.idx}'/>)"><c:out value='${news.news_title}'/></a>
			                                    </td>
			                                    ...
			                                    <td class="center" style="width: 100px;"><c:out value='${news.upd_date}'/></td>
			                                </tr>
		                                </c:forEach>
		                            </tbody>
		                        </table>
...
<div id="dialog-view" title="뉴스 상세표시">
