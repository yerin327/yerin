...
<section class="form_area">
	<div class="tab_area">
    	 <ul>
            ...
            <li><img src="<%=request.getContextPath() %>/images/locale/ja/user/pages/school/recommended_school_on.png" alt="recommended_school" width="186" height="50" border="0"></li>
		                            ...
		                            <tbody>
		                            	<c:forEach var="school" items="${school_list }" varStatus="status">
			                                ...
			                                <tr class="<c:out value='${rowClass}'/>">
			                                    <td style="width: auto; ">
			                                    	<a href="<%=request.getContextPath() %>/user/school/schoolDetail.do?sb_id=<c:out value='${school.sb_id }'/>"><c:out value='${school.school_name}'/></a>
			                                    </td>
			                                    <td style="width: 120px;">
			                                    	<c:choose>
				                                    	<c:when test="${school.rcmmd_type == 'LOC'}">희망지역</c:when>
				                                    	<c:when test="${school.rcmmd_type == 'SCH'}">희망학교</c:when>
				                                    	<c:when test="${school.rcmmd_type == 'BAC'}">기본정보</c:when>
			                                    	</c:choose>
			                                    </td>
