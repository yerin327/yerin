...
public class ScheduleController {
	...
	@RequestMapping("/user/schedule/calendar")
	...
	@RequestMapping("/user/schedule/scheduleAll")
	...
	@RequestMapping("/user/schedule/scheduleRcmmd")
	public ModelAndView scheduleRcmmd(HttpServletRequest request,
										HttpSession session,
										Model model) {
		ModelAndView mav = new ModelAndView();
		UserEntity userEntity = (UserEntity)session.getAttribute("login_user");
	...
	@RequestMapping("/user/schedule/scheduleJson")
    public void jsonpTest(HttpServletRequest request,
		...
		String isRcmmd = request.getParameter("isRcmmd");
		...
		} else if (isRcmmd != null && isRcmmd.equals("yes")) {
			scheduleEntity.setStudent_id(request.getParameter("student_id"));
			scheduleList = scheduleService.listOfRcmmd(scheduleEntity);
		} else {
			scheduleEntity.setStudent_id(request.getParameter("student_id"));
	...
	@RequestMapping("/user/schedule/scheduleMonthly")
	public ModelAndView scheduleMonthly(HttpServletRequest request,
		...
		if (userEntity != null) {		// login 페이로부터 이동 
			...
			mav.setViewName("/user/schedule/scheduleMonthly");
		} else {
			mav.setViewName("redirect:/index.do?resultCode=" + ResultCode.USER_ERR_NOTLOGIN_CODE);
		}
	...
	@RequestMapping("/user/schedule/scheduleWeekly")
	public ModelAndView scheduleWeekly(HttpServletRequest request,
		...
		if (userEntity != null) {		// login 페이로부터 이동 
			...
			mav.setViewName("/user/schedule/scheduleWeekly");
		} else {
			mav.setViewName("redirect:/index.do?resultCode=" + ResultCode.USER_ERR_NOTLOGIN_CODE);
		}
	...
	@RequestMapping("/user/schedule/scheduleDaily")
	public ModelAndView scheduleDaily(HttpServletRequest request,
		...
		if (userEntity != null) {		// login 페이로부터 이동 
			...
			scheduleEntity.setStudent_id(userEntity.getUser_id());
			scheduleEntity.setEvt_date_s(calendar[0] + "-" + 
										((calendar[1] + 1) < 10 ? "0" + (calendar[1] + 1) : "" + (calendar[1] + 1)) + "-" + calendar[2]);
			
			List<ScheduleEntity> scheduleList = scheduleService.listOneDay(scheduleEntity);
			Map<String, List<ScheduleEntity>> eventMap = new HashMap<String, List<ScheduleEntity>>();
			for (ScheduleEntity sEntity : scheduleList) {
				List<ScheduleEntity> eventOfDay = null;
				
				String key = sEntity.getEvt_time_s();
				
				if (eventMap.containsKey(key)) {
					eventOfDay = eventMap.get(key);
				} else {
					eventOfDay = new ArrayList<ScheduleEntity>();
				}
				
				eventOfDay.add(sEntity);
				eventMap.put(key, eventOfDay);
			}
			
//			Iterator<String> it = eventMap.keySet().iterator();
//			while (it.hasNext()) {
//				String day = it.next();
//				List<ScheduleEntity> list = eventMap.get(day);
//				System.out.println(day + "={");
//				for (ScheduleEntity sEntity : list) {
//					System.out.println(sEntity.getEvt_title() + "/" + sEntity.getEvt_date_s() + "T" + sEntity.getEvt_time_s() + "~" + 
//																	  sEntity.getEvt_date_e() + "T" + sEntity.getEvt_time_e());	
//				}
//				System.out.println("}");
//			}
			
			mav.addObject("event_map", eventMap);
			mav.setViewName("/user/schedule/scheduleDaily");
		} else {
			mav.setViewName("redirect:/index.do?resultCode=" + ResultCode.USER_ERR_NOTLOGIN_CODE);
		}
	...
	@RequestMapping("/user/schedule/scheduleAddProc")
...
//			} else {
//				obj.put("result_code", ResultCode.EVENT_ERR_EVENT_CODE);
//				obj.put("result_msg", ResultCode.EVENT_ERR_EVENT_MSG);
//			}
...
//			mav.setViewName("redirect:/index.do");
	...
	@RequestMapping("/user/schedule/eventRegProc")
    public void eventRegProc( Model model,
    								  HttpServletRequest request,
									  HttpSession session,
									  HttpServletResponse response) {
		String callback = request.getParameter("jsoncallback");
		...
		if (userEntity != null) {
			...
			if("TRUE".equals(request.getAttribute("TOKEN_SAVE_CHECK"))) {
				...
				//scheduleService.add(scheduleEntity);
				System.out.printf(event_id);
				obj.put("result_code", ResultCode.EVENT_E_SUCCESS_CODE);
				obj.put("result_msg", ResultCode.EVENT_E_SUCCESS_MSG);
			} else {
				obj.put("result_code", ResultCode.EVENT_ERR_EVENT_CODE);
				obj.put("result_msg", ResultCode.EVENT_ERR_EVENT_MSG);
			}
...
//        return mav;
	...
	@RequestMapping("/user/schedule/{evt_id}")
